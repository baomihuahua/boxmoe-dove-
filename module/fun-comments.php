...
<?php if ( $args['type'] != 'pings' ) : ?>
                                                      ...
                                                      <div class="comment-body">
                                                            ...
                                                            <?php if($comment->user_id > 1){
																echo '<span class="commentsbadge members"><i class="fa fa-heart"></i> '.boxmoe_com('comnanesu').'</span>'; 
																}if($comment->user_id == 1){
																echo '<span class="commentsbadge blogger"><i class="fa fa-check-square"></i> '.boxmoe_com('comnanes').'</span>'; 
																}if($comment->user_id == 0){
																echo '<span class="commentsbadge tourists"><i class="fa fa-globe"></i> '.boxmoe_com('comnaness').'</span>'; }
																?>
<?php echo get_comment_reply_link( array('depth'      => $depth,'max_depth'  => $args['max_depth'],
'reply_text' => sprintf( '<span class="pull-right"><i class="fa fa-mail-reply-all"></i> %s </span>', esc_html__( '回复', 'boxmoe' ).'' ),),
$comment->comment_ID,$comment->comment_post_ID); ?>
							...
							/*说明: %1$s 是日期, %2$s 是时间 */
							esc_html__( '%1$s | %2$s', 'boxmoe' ),get_comment_date(),get_comment_time());edit_comment_link( esc_html__( '(编辑)', 'boxmoe' ), '  ', '' );?></span>
                                                            </div>
...
$comnanesgo = boxmoe_com('comnanesgo');
if(!empty($comnanesgo)) $comnanesgo_url = boxmoe_com('comnanesgo_url');else $comnanesgo_url ='';
...
function boxmoe_comment_add_at( $comment_text, $comment = '') {  
  if( $comment->comment_parent > 0 & $comment->user_id == 1) {  
    $comment_text = '<span class="commentsbadge badge-pill atwho text-uppercase">@'.get_comment_author( $comment->comment_parent ) . '</span> ' . $comment_text.'';  
  }  
  if( $comment->comment_parent > 0 & $comment->user_id != 1) {  
    $comment_text = '<span class="commentsbadge badge-pill atwho text-uppercase">@'.get_comment_author( $comment->comment_parent ) . '</span> ' . $comment_text;  
  }
...
<div style="border-radius: 10px 10px 10px 10px;font-size:13px;    color: #555555;width: 99%;font-family:"Century Gothic","Trebuchet MS","Hiragino Sans GB",微软雅黑,"Microsoft Yahei",Tahoma,Helvetica,Arial,"SimSun",sans-serif;margin:50px auto;border:1px solid #eee;max-width:100%;background: #ffffff repeating-linear-gradient(-45deg,#fff,#fff 1.125rem,transparent 1.125rem,transparent 2.25rem);box-shadow: 0 1px 5px rgba(0, 0, 0, 0.15);">
